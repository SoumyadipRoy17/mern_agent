...
const uploadList = async (req, res) => {
  ...
  let insertedCount = 0; // Track successful insertions
  ...
  for (const row of data) {
    ...
    const existingRecord = await List.findOne({ phone });

    if (existingRecord) {
      console.warn(`Skipping duplicate record for phone: ${phone}`);
  ...
  res
    .status(200)
    .json({ message: `List uploaded. ${insertedCount} new records added.` });
};
